...
OBJECT(instruction, string* symbol; string* opcode; string* operand; string* comment);
...
bool isOPCode(string* who)
{
	if (!VALID(who))
		return false;
	for (int i = 0; i < totalInstructions; ++i)
		if (strcmp(who->c_str, instructions[i]) == 0)
			return true;
	return false;
}
...
{
	return (!isComment(who) && !isDirective(who) && !isOPCode(who));
}
...
bool pass1(vector* lines)
{
	...
	bool explicitStart = false; bool addressExceeded = false;  bool explicitEnd = false; unsigned long start = 0; unsigned int totalInstructions = 0;
	...
	for (unsigned int i = 0; i < lines->num; ++i)
	{
		...
		if (text->length <= 1) //ignore last line potential whitespace.
		{
			if (i != lines->num - 1)
			...
			{
				vector_push_back(warnings, (object*)string_make_and_format("%sINSTRUCTION ON LINE %s%i%s IS AFTER %sEND%s AND IS IGNORED%s", YELLOW, LIGHT_CYAN, line, YELLOW, LIGHT_CYAN, YELLOW, NEWLINE));
				continue;
			}
			if (!addressExceeded && accumulator >= 0x8000)
			{
				...
				vector_push_back(errors, (object*)string_make_and_format("%sMAXIMUM ADDRESSABLE MEMORY EXCEEDED %s%X%s >= %s%X%s BY LINE %s%i%s!%s",
					LIGHT_RED, LIGHT_CYAN, accumulator, LIGHT_RED, LIGHT_CYAN, 0x8000, LIGHT_RED, LIGHT_CYAN, line, LIGHT_RED, NEWLINE));
			}
			...
			case 1:
				if (strcmp(parsed->opcode->c_str, "START") == 0)
				{
					...
					else if (!fromHex(parsed->operand->c_str, &accumulator))
					{
			...
			default:
				if (parsed->symbol->length != 0)
				{
					...
					}else if (hashTable_has(symtab, parsed->symbol->c_str))
					{
						bucket* dupe = hashTable_get(symtab, parsed->symbol->c_str);
						...
						pair* data = NEW(pair);
						pair_make(data, line, accumulator); //line, address
						hashTable_insert(symtab, parsed->symbol->c_str, data);
						DELETE(data);
					};
					if (errors->num == 0) /* no point in adding to this symbol table, we already have a fatal error. */
						vector_push_back(symbols, (object*)string_make_and_format("│ %s%-8s%s│ %s%04lX %s│%s", YELLOW, parsed->symbol->c_str, RESET, LIGHT_CYAN, accumulator, RESET, NEWLINE));
				}
				if (parsed->opcode->length != 0)
				{
					if (isOPCode(parsed->opcode))
					{
						accumulator += 3; //initial increase...
					}
					else if (isDirective(parsed->opcode))
					{
						...
						else if (strcmp(parsed->opcode->c_str, "WORD") == 0)
						{
							...
							else
							{
								...
								{
									vector_push_back(warnings, (object*)string_make_and_format(
						...
						else if (strcmp(parsed->opcode->c_str, "BYTE") == 0)
						{
							...
							{
								accumulator += strlen(opData->c_str); //check the chars maybe ? idk
							}
								...
								else
								{
									...
									accumulator += (length / 2) + (length % 2); //round-up to nearest multiple of 2. FFF -> 0F FF NOT FF!!
								}
							}
							DELETE(op);
						}
	...
	if (!explicitStart)
		vector_push_back(warnings, (object*)string_make_and_format("%sSTART DIRECTIVE MISSING %sSTART —> 0%s", YELLOW, LIGHT_CYAN, NEWLINE));

	if (!explicitEnd)
		vector_push_back(warnings, (object*)string_make_and_format("%sEND DIRECTIVE MISSING %sEND —> LAST INSTRUCTION%s", YELLOW, LIGHT_CYAN, NEWLINE));
	...
	if (errors->num == 0)
	{
		printf("%sProgram Length %lX%s", LIGHT_CYAN, accumulator - start, NEWLINE);
	}
	...
	DELETE(symbols); DELETE(errors); DELETE(warnings); DELETE(symtab);

	return passed;
}
bool pass2(vector* lines)
{
...
int MAIN(int argc, char* argv[])
{
	...
	if (!pass1(lines))
	{
		...
		printf("%sPASS 1 SUCCESS!%s", GREEN, NEWLINE);
		//pass 2 here
	}
